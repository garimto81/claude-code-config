...
**문서 버전**: 3.6.2 | **최종 업데이트**: 2025-01-16 | **5분 완독용**
...
|------|------|------|
| 핸드 기록 완료율 | 95% | 93% |
| **VIRTUAL 전송 성공률** | **98%** | **100%** ⭐ |
| 평균 핸드 기록 시간 | < 3분 | 2.8분 |
| **초기 로딩 시간** | **< 1초** | **0.475초 (76% 절감)** ⭐ |
| **VIRTUAL 전송 시간** | **< 5초** | **2.0초 (56% 개선)** ⭐ |
| **UX 개선** | **깜빡임 제거** | **300ms 미만 작업 로딩 숨김** ⭐ |
| 지원 테이블 수 | 36개 | 36개 ✅ |
...
### v3.6.2 (2025-01-16) - Critical Bug Fix: stacks_json 데이터 정렬 오류 수정
| 이슈 | 상태 | 조치 |
|------|------|------|
| **stacks_json 데이터 정렬 오류** | Critical | ✅ 데이터 정렬 로직 수정 |
| **bb_amount 컬럼 불일치** | High | ✅ 컬럼 제거 (CSV 스키마 통일) |
| **Review 탭 플레이어 표시** | Medium | ✅ holes_json 기반 정보 추출 |

**근본 원인**:
- stacks_json 데이터가 좌석 번호 순서대로 정렬되지 않아 오류 발생
- bb_amount 컬럼이 CSV 스키마와 불일치하여 데이터 불일치 발생
- Review 탭에서 stacks_json 없는 경우 플레이어 정보 표시 안 됨

**해결 (v3.6.2)**:
```javascript
// stacks_json 데이터 정렬 로직 수정
const sortedStacks = Object.keys(stacks).sort().reduce((obj, key) => {
  obj[key] = stacks[key];
  return obj;
}, {});

// bb_amount 컬럼 제거 (CSV 스키마와 통일)
// Review 탭에서 holes_json 기반으로 플레이어 정보 추출
```

**영향도**: Review 탭 정상 작동 + 데이터 일관성 보장

---

### v3.6.0 (2025-01-16) - 스마트 적응형 로딩 UI (Smart Adaptive Loading)
| 항목 | Before | After |
|------|--------|-------|
| **로딩 표시 기준** | 모든 작업 무조건 표시 | 300ms 미만 작업 자동 숨김 ⭐ |
| **로딩 UI 타입** | 단일 오버레이 (불투명) | Compact 모드 지원 (75% 투명) |
| **햅틱 피드백** | 수동 호출 | 중요 작업 자동 적용 ⭐ |
| **코드 복잡도** | 다중 함수 (showSpinner, hideSpinner 등) | 단일 함수 (showLoading/hideLoading) |

**핵심 개선:**
- ✅ **Micro-Delay 패턴**: 300ms 미만 작업 깜빡임 제거 (체감 속도 67% 단축)
- ✅ **Compact 모드**: Review/커밋용 경량 오버레이 (배경 유지)
...
// After (v3.6.0)
showLoading('로딩 중...', { compact: true, haptic: 'MEDIUM' });
const data = await someAPI();
hideLoading();
```
...
### v3.5.0 (2025-01-15) - Sparse Column Reads (2차 성능 최적화)
| 항목 | Before | After | 개선 |
|------|--------|-------|---------|
| **queryHands() 컬럼 읽기** | 20개 전체 | 11개 필수 | 45% 절감 ⭐ |
| **Review 탭 로딩** | 500ms | 275ms | 45% 단축 |
...
### Type 시트 스키마 (v2.8.0 확장)
**표준 헤더** (11개 컬럼):
```
PokerRoom, TableName, TableId, TableNo, SeatId, SeatNo, PlayerId, PlayerName, Nationality, ChipCount, key player
```

**컬럼 구분:**
- 필수 6개: TableNo, SeatNo, PlayerName, Nationality, ChipCount, key player
- 선택 5개: PokerRoom, TableName, TableId, SeatId, PlayerId ⭐
...
**VIRTUAL 시트 자동화:**
- 파일명: 자동 생성 (시간, 핸드번호, 플레이어 정보)
- 히스토리: 3줄 요약 (참가자, 보드, 팟)
- 키플레이어 자막: 이름/국적/스택 또는 ELIMINATED
- K열: "버추얼 테이블" 자동 입력 (v3.3.4)
...
- **스마트 적응형 로딩 UI (v3.6.0): 깜빡임 제거** ⭐
- **VIRTUAL K열 자동화 (v3.3.4)**
- **핸드별 BB 저장 (v3.3.3)**
...
- 기여액 기반 **정확한 팟 계산**
- 키플레이어 **자동 자막 생성**
- **v3.4.0+v3.5.0+v3.6.0**: 3단계 성능 최적화 + UX 개선으로 **누적 80% 개선** ⭐
